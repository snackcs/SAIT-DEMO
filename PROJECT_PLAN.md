...
- [ ] Настроить форматирование кода — создать `.editorconfig` (indent_size=2, end_of_line=lf — обязательно для Windows/Mac совместимости) и `prettier.config.js`:
  ```js
  // prettier.config.js
  module.exports = { semi: false, singleQuote: true, trailingComma: 'es5', printWidth: 100 }
  ```
  Добавить скрипт `"format": "prettier --write src/"` в `package.json`
...
- [ ] OG-изображение для каждой страницы ниши — через `generateMetadata()` в `page.tsx`:
  - `title`, `description`, `openGraph.images`
  - **`alternates.canonical`** — обязательно указать абсолютный URL страницы:
    ```ts
    alternates: { canonical: `${process.env.NEXT_PUBLIC_SITE_URL}/${data.slug}` }
    ```
    Без canonical Google может посчитать несколько версий URL дублями и понизить в выдаче
  - OG-изображения: использовать статичный `data.seo.ogImage` (Unsplash URL). Динамический `next/og` — только если нужен брендированный текст на картинке, усложняет проект без необходимости
- [ ] `src/app/robots.ts` — Next.js генерация `robots.txt`:
  - Все страницы индексировать — демо-страницы приносят органический трафик из ниш
- [ ] `src/app/sitemap.ts` — автогенерация `sitemap.xml` с `process.env.NEXT_PUBLIC_SITE_URL`
...
- [ ] Форма через **Formspree** (не mailto!):
  ...
  - **Клиентская валидация** (до отправки): обязательные поля, мин. длина имени 2 символа, поле телефона/Telegram мин. 5 символов
  - **Обработка ответа Formspree**:
    - `succeeded === true` → зелёный блок "Заявка получена! Свяжусь в течение часа."
    - `errors` (объект с ошибками полей) → подсветить конкретные поля с текстом ошибки под ними (Formspree возвращает `errors[field].message`)
    - Сетевая ошибка / 500 → красный блок "Что-то пошло не так. Напишите напрямую в Telegram."
    - **429 (лимит исчерпан)** → красный блок "Форма временно недоступна. Напишите напрямую." + контакты Telegram/Email на виду
  - Состояния кнопки отправки: дефолт / spinner "Отправляем..." (disabled) / успех
  - ⚠️ **Free tier: 50 submissions/month.** При росте трафика — перейти на платный план ($8/мес) или подключить спам-фильтр в настройках Formspree (reCAPTCHA интеграция)
- [ ] `WhatNext` — 3 шага
...
- [ ] ⚠️ **ФЗ-152 / cookie consent**: Яндекс.Метрика пишет cookies без согласия пользователя, что формально нарушает ФЗ-152. Реализация:
  - Создать компонент `src/components/CookieBanner.tsx`
  - Баннер показывается внизу экрана, если `localStorage.getItem('cookie_consent')` не равен `'accepted'`
  - При клике "Понятно" → `localStorage.setItem('cookie_consent', 'accepted')` + скрыть баннер
  - **Яндекс.Метрика загружается только после согласия**: в `YandexMetrika.tsx` проверить `localStorage.getItem('cookie_consent') === 'accepted'` перед рендером `<Script>`; или использовать `strategy="lazyOnload"` и условный рендер
  - Текст баннера: "Мы используем cookies для аналитики сайта. Продолжая использование, вы соглашаетесь." + кнопка [Принять]
  - Добавить `CookieBanner` в `app/layout.tsx` рядом с `YandexMetrika`
...
> Полная дизайн-система описана в **[DESIGN.md](./DESIGN.md)** — там приоритетные значения.
> Ниже — краткая выжимка для быстрого доступа.

### Цветовая палитра (глобальная) — светлая тема

| Токен            | Цвет              | Назначение                        |
|------------------|-------------------|-----------------------------------|
| background       | #F8FAFC           | Фон сайта (светло-серый, не белый)|
| surface          | #FFFFFF           | Фон карточек                      |
| surface-alt      | #F1F5F9           | Альтернативный фон секций         |
| border           | #E2E8F0           | Границы элементов                 |
| text-primary     | #0F172A           | Основной текст                    |
| text-secondary   | #475569           | Вторичный текст                   |
| hero-overlay     | rgba(10,15,25,0.58)| Тёмный оверлей Hero/CTA секций   |
